...
const RAW_OWNERS = [
  ...
  {"id":68,"sn":68,"nm":"장영훈","addr":"문정동 28-15 청구아트빌라 301호","tp":"연립주택","cat":"공동주택","area":51.1,"asset":652000000,"rights":992278800,"agreed":false,"age":"50대","fullAddr":"서울특별시 송파구 정보센터","residing":false,"items":1},
  ...
  {"id":81,"sn":81,"nm":"이영숙","addr":"문정동 28-21 청기와아트빌라 205호","tp":"연립주택","cat":"공동주택","area":52.8,"asset":569000000,"rights":865961100,"agreed":false,"age":"60대","fullAddr":"서울특별시 송파구 정보센터","residing":false,"items":1},
...
export default function App() {
  ...
  // [SYNC] 기존 100명 명단을 서버에 처음 한 번 등록하는 함수
  const syncInitialData = async () => {
    if (!window.confirm("기존 데이터를 서버에 동기화하시겠습니까?")) return;
    try {
    ...
    <div className="app-wrapper">
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #0C0C0E; font-family: 'Pretendard', sans-serif; overflow: hidden; }
            ...
            <header style={{ padding: "16px 20px 12px", background: "linear-gradient(180deg, #0C0C0E 0%, transparent 100%)", position: "sticky", top: 0, zIndex: 50 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <button onClick={syncInitialData} style={{ opacity: 0.1, fontSize: 8, color: "#fff", background: "none", border: "1px solid #fff", borderRadius: 4, padding: "2px 4px" }}>SYNC</button>
...
function InteractiveSvgMap({ owners, onSelect, mapFilter, setMapFilter, stats, isDesktop = false, initialScale = 0.45 }) {
  ...
  const lotData = useMemo(() => {
    ...
    Object.entries(BASE_POLYS).forEach(([jibun, p]) => {
      ...
      lots[jibun] = { jibun, center: MANUAL_CENTERS[jibun] || [xSum, ySum], polygon: p, owners: ownersOnLot, totalCount, agreedCount, primaryCat: ownersOnLot.length > 0 ? ownersOnLot[0].cat : "기타", allAgreed: totalCount > 0 && agreedCount === totalCount, someAgreed: agreedCount > 0 && agreedCount < totalCount, noneAgreed: totalCount > 0 && agreedCount === 0, isEmpty: totalCount === 0 };
    });
    return lots;
  }, [owners]);
    ...
    <div style={{ height: '100%', position: 'relative', overflow: 'hidden', background: '#0a0a0a', borderRadius: 16, border: '1px solid #2A2A2E' }} onMouseDown={handlePointerDown} onMouseMove={handlePointerMove} onMouseUp={() => dragRef.current.isDragging = false} onTouchStart={handlePointerDown} onTouchMove={handlePointerMove} onTouchEnd={() => dragRef.current.isDragging = false}>
      ...
      {selectedLot && (
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: '#fff', padding: 20, borderRadius: 16, color: '#000', width: 260, boxShadow: '0 10px 30px rgba(0,0,0,0.5)', zIndex: 100 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}><h3>문정동 {selectedLot.jibun}</h3><button onClick={() => setSelectedLot(null)} style={{ border: 'none', background: 'none', fontSize: 18 }}>✕</button></div>
...
function DetailView({ owner, onBack, updateOwner, currentUser }) {
  ...
  return (
    <div style={{ padding: "20px", color: "#E8E6E1", overflowY: 'auto', height: '100vh' }}>
      <button onClick={onBack} style={{ color: "#FF2A55", background: "none", border: "none", marginBottom: 16 }}>← 뒤로가기</button>
      ...
      <div style={{ background: "#161618", padding: 20, borderRadius: 16, marginTop: 16, border: "1px solid #1E1E22" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 20 }}>
          <button onClick={() => updateOwner(owner.id, { agreed: true, agreedBy: currentUser, consentDate: new Date().toISOString().split('T')[0] })} style={{ padding: 14, borderRadius: 12, border: "none", background: owner.agreed ? "#22C55E" : "#2A2A2E", color: "#fff", fontWeight: 700 }}>동의 완료</button>
          <button onClick={() => updateOwner(owner.id, { agreed: false })} style={{ padding: 14, borderRadius: 12, border: "none", background: !owner.agreed ? "#EF4444" : "#2A2A2E", color: "#fff", fontWeight: 700 }}>미동의</button>
        </div>
      </div>

      <div style={{ background: "#161618", padding: 20, borderRadius: 16, marginTop: 16, border: "1px solid #1E1E22" }}>
        <h3 style={{ fontSize: 14, marginBottom: 12 }}>분담금 시뮬레이션</h3>
        <InputRow label="종전자산 가액" value={asset.toLocaleString()} onChange={e => setAssetStr(e.target.value)} suffix="원" />
      ...
      <div style={{ marginTop: 24, paddingBottom: 100 }}>
        ...
        <textarea value={newMemo} onChange={e => setNewMemo(e.target.value)} placeholder="내용 입력..." style={{ width: "100%", height: 100, background: "#1E1E22", border: "1px solid #2A2A2E", borderRadius: 12, color: "#fff", padding: 12 }} />
        <button onClick={handleAddMemo} style={{ width: "100%", padding: 12, background: "#FF2A55", color: "#fff", border: "none", borderRadius: 12, marginTop: 8, fontWeight: 700 }}>메모 추가</button>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        {["전체", "미동의", "동의완료"].map(f => <button key={f} onClick={() => setFilter(f)} style={{ padding: "8px 16px", borderRadius: 20, border: "none", background: filter === f ? "#FF2A55" : "#1E1E22", color: "#fff", fontSize: 12 }}>{f}</button>)}
      </div>
      {filtered.map(o => (
...
function CategoryView({ stats, catTab, setCatTab }) {
  ...
  return (
    <div style={{ background: "#161618", padding: 20, borderRadius: 16, border: "1px solid #1E1E22" }}>
      <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>{["공동주택", "단독/다가구", "상가/기타"].map(c => <button key={c} onClick={() => setCatTab(c)} style={{ flex: 1, padding: 10, background: catTab === c ? "#2A2A2E" : "none", color: "#fff", border: "1px solid #2A2A2E", borderRadius: 8 }}>{c}</button>)}</div>
...
function InputRow({ label, value, onChange, suffix }) { return <div style={{ marginTop: 12 }}><p style={{ fontSize: 11, color: "#9CA3AF", marginBottom: 6 }}>{label}</p><div style={{ position: "relative" }}><input value={value} onChange={onChange} style={{ width: "100%", padding: 12, background: "#1E1E22", border: "1px solid #2A2A2E", borderRadius: 10, color: "#fff", textAlign: "right" }} /><span style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: "#9CA3AF", fontSize: 12 }}>{suffix}</span></div></div>; }